...
const ShowtimesPage: React.FC = () => {
  const [showtimes, setShowtimes] = useState<
    {
      ...
      theater: { id: number; name: string; numberOfRows: number; numberOfColumns: number };
      startTime: string;
      endTime: string;
      price: number;
      bookedCount: number;
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_API_BASE_URL}/showtimes`)
      .then((response) => {
  ...
  const handleDelete = (id: number) => {
    ...
    if (isConfirmed) {
      axios
        .delete(`${process.env.REACT_APP_API_BASE_URL}/showtimes/${id}`)
        .then(() => {
  ...
  const handleUpdateShowtime = (updatedShowtime: any) => {
    axios
      .post(`${process.env.REACT_APP_API_BASE_URL}/showtimes/update/${updatedShowtime.id}`, updatedShowtime)
      .then(() => {
        axios
          .get(`${process.env.REACT_APP_API_BASE_URL}/showtimes/${updatedShowtime.id}`)
          .then((response) => {
  ...
  return (
    <div className="showtimes-container">
                ...
                <TableRow key={showtime.id}>
                  ...
                  <TableCell className="buy-tickets-button">
                    <button
                      ...
                      disabled={
                        showtime.theater.numberOfColumns * showtime.theater.numberOfRows ===
                        showtime.bookedCount
                      }
                    >
                      <p className="buy-tickets-text" onClick={() => handleOpenBooking(showtime)}>
                        Buy Tickets
                      </p>
                    </button>
                  </TableCell>
                  <TableCell>
                    {(() => {
                      ...
                      const seatsByPercentage = (showtime.bookedCount / totalSeats) * 100;

                      if (seatsByPercentage === 100) {
                        return <span className="text-danger"><strong>Sold Out</strong></span>;
                      } else if (seatsByPercentage >= 90) {
                        return <span className="text-danger">Last Seats</span>;
                      } else if (seatsByPercentage >= 33) {
                  ...
                  <TableCell align="right">
                    <Tooltip title={showtime.bookedCount > 0 ? "Cannot edit a showtime with booked tickets" : "Edit"}>
                      <span>
                        <button
                          className="edit-showtime-btn"
                          onClick={() => handleEdit(showtime)}
                          disabled={showtime.bookedCount > 0}
                        >
                          ✏️
                        </button>
                      </span>
      ...
      <EditShowtimeModal
        ...
        handleUpdate={handleUpdateShowtime}
