...
const EditShowtimeModal: React.FC<EditShowtimeModalProps> = ({
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    const parsedValue =
      name === "movieId" || name === "theaterId" || name === "price"
        ? parseInt(value, 10)
        : value;
  ...
  const isFormInvalid = Object.values(showtimeData).some(
    (value) => value === "" || value === 0
  );
  ...
  const handleSubmit = () => {
    ...
    if (movieDuration === null) {
      setError("⏳ Please wait for the movie duration to load.");
      return;
    }
